...
pub class Workflow {
  ...
  errorCallbacks: MutArray<inflight (Error): void>;

  new(steps: Array<Step>, opts: WorkflowOptions?) {
    let debug = opts?.debug ?? false;

    this.steps = steps;
    this.successCallbacks = MutArray<inflight (Json?): void>[];
    this.errorCallbacks = MutArray<inflight (Error): void>[];
  ...
  pub onError(cb: inflight (Error): void) {
  ...
  pub inflight status(): seq.Status {
  ...
  pub inflight error(): Error? {
    return Error.tryFromJson(this.store.getError());
  }
  ...
  pub inflight join(opts: util.WaitUntilProps?): Json {
    util.waitUntil(
      inflight () => { 
        ...
        return this.status() == seq.Status.DONE;
      },
      opts
    );
  ...
  addExecuteStep(prev: Array<std.IResource>, step: ExecuteStep, opts: WorkflowOptions): Array<std.Resource> {
    let handler = new cloud.Function(inflight (ctx) => {
      ...
      let merged = Workflow.mergeJson(inp, output);
  ...
  addCheckStep(prev: Array<std.IResource>, step: CheckStep, opts: WorkflowOptions): Array<std.IResource> {
    let checkStep: CheckStep = unsafeCast(step);
  ...
  extern "./util.ts" static inflight mergeJson(a: Json, b: Json): Json;
}
